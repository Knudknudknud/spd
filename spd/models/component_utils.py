...
def calc_causal_importances(
    ...
    active_module = gates.get("active_module", None)
    prev_gate_output = None #set to none if we require previous activations, also add it under active module key.
    ...
    for param_name in pre_weight_acts:
        ...
        if active_module is not None and prev_gate_output is not None:
            gate_output = active_module(gate_input, prev_gate_output)
            #gate_output = active_module(gate_input)
        else:
            gate_output = gates[param_name](gate_input)
