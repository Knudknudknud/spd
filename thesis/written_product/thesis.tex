...
\end{table*}
\subsection{SPD Baseline}
\subsubsection{Toy Model of Superposition}

The Toy Model of Superposition (TMS) \autocite{elhage2022toymodelssuperposition} is trained to reconstruct sparse feature activations, random vectors in $\mathbb{R}^{d_{\text{features}}}$ where each entry is drawn independently from a uniform distribution over $[0, 1]$. These features are passed through a lower-dimensional bottleneck that forces the model to learn features in superposition. The architecture is a single-hidden-layer autoencoder with tied weights:
\begin{equation}
    \hat{x} = \mathrm{ReLU}(W^{\top}Wx + b), \qquad W \in \mathbb{R}^{d_{\text{hidden}} \times d_{\text{features}}}, \quad d_{\text{features}} \gg d_{\text{hidden}}.
\end{equation}
As each feature is independent and one-dimensional, the ground-truth decomposition is a set of $d_{\text{features}}$ rank-one matrices, each non-zero only along the $c$-th column $W_{:,c}$ of the target weights.

To quantify how well these features are recovered, the learned subcomponents are compared against the target model in terms of both directional alignment and magnitude. The mean max cosine similarity (MMCS) measures directional alignment between each target feature and its best-matching learned subcomponent:
\begin{equation}
\text{MMCS}
=
...
Apart from the directional alignment, the strength of the alignment is also computed, to see if the method is sensitive to feature shrinkage,
a problem that is apparent in both Sparse Autoencoders and Attribution-Based Parameter Decomposition \autocite{jermyn2024tanhPenaltyDictionaryLearning,wright2024featureSuppressionSAEs,AttributionBasedParameterDecomposition}.
To quantify feature shrinkage, the mean $L_{2}$ ratio (ML2R) is computed between the euclidean norm of the best-aligned subcomponent and the target feature.
\begin{equation}
\text{ML2R}
=
\frac{1}{d_{\text{features}}}
...
Like in the original SPD paper, two TMS models are trained: $\mathrm{TMS}_{5\text{-}2}$ with $d_{\text{features}}=5$ and $d_{\text{hidden}}=2$, and $\mathrm{TMS}_{40\text{-}10}$ with $d_{\text{features}}=40$ and $d_{\text{hidden}}=10$ with an additional identity layer. For these experiments the matrices are kept at rank $1$, as this is both the ground-truth structure and isolates the effect of softmax.
...
\newpage
SPD achieves near perfect alignment on both the small and large TMS models, with MMCS and ML2R approximately equal to $1$. The softmax and attention variant, while achieving near-zero losses, does not concentrate the entire feature magnitude within a single subcomponent, resulting in a lower ML2R with high variance. For the larger model this skews the geometry of the decomposition further (Table~\ref{tab:tms_baseline} and Figure~\ref{fig:tms_5_2}).
...
\midrule
MMCS  & $1.00$ & $0.985 \pm 0.016$ & $1.00$ & TBD \\
ML2R  & $1.00$ & $0.797 \pm 0.205$ & $1.055 \pm 0.045$ & TBD \\
\bottomrule
\end{tabular}
\caption{Decomposition quality on the TMS baseline. SPD refers to the original method \autocite{stochasticparameterdecomposition}, while Softmax refers to the relational importance function of Section~\ref{sec:rank-k-attention}.}
\label{tab:tms_baseline}
\end{table}
While softmax does not recover the exact geometry,
it does manage to correctly concentrate the entire magnitude on the correct number of subcomponents, with the remaining subcomponents pruned to near-zero.
In terms of the causal importance values, the softmax and original SPD method show 
the same onehot assignment for each of the five features in $\mathrm{TMS}_{5\text{-}2}$, 
with some degradation in the larger model, where certain subcomponents are causally important for multiple features.
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth, height=7cm]{tms_5_2_plot_causal_importances.jpg}
    \caption{Causal importance values for the softmax on $\mathrm{TMS}_{5\text{-}2}$, showing a one-hot assignment for each of the five features.}
\end{figure}
...
a single-hidden-layer autoencoder with tied weights (For a detailed description see Section~\ref{sec:spd-independent-features}):
\begin{equation}
    f(x) = W^{\top} \sigma(Wx + b), \qquad W \in \mathbb{R}^{d_{\text{hidden}} \times d_{\text{features}}}, \quad d_{\text{features}} \gg d_{\text{hidden}}
\end{equation}
Like before, the model is trained to reconstruct sparse features.
However this time, the features are sampled in pairs, creating statistical dependence and thus irreducibility (Section~\ref{sec:irreducibility})
features. The ground-truth decomposition thus becomes $d_{features} / 2$ matrices, each non-zero along two columns of the target weights, corresponding to the paired features.
Concretely, for this experiment, $d_{features}=40$ and $d_{hidden}=10$, with the features partitioned into 20 pairs $\{f_{2i-1}, f_{2i}\}_{i=1}^{20}$. Each pair is zero with probability $1-p$, and with probability $p$ both features are independently drawn from $\mathcal{U} \sim (\epsilon, 1)$.
Zero is reserved to indicate the absence of a feature.
Interestingly, the target TMS model learns to group the paired features along a single direction, with near-perfect collinearity in the groups, and near-orthogonal directions across groups (Table~\ref{tab:tms-paired-cosine-similarity}).
...
As described in the multi-dimensional superposition hypothesis,
the dimension of the input feature specifies an upper bound on the rank required to represent it, 
but does not enforce the model to use all dimensions.
In this case, the model learns to represent it in a one-dimensional subspace.
The SPD decomposition should thus be able to capture each pair within a single rank-one subcomponent, without needing to split the features across multiple subcomponents.

This therefore acts as the most minimal extension to SPD, same setup, multidimensional features that collapse into 1D.
We keep the subcomponents at rank 1, as this is the ground-truth structure and isolates the differences between softmax and SPD.

but the observed near-collinearity suggests that the model learns to represent each pair along a single direction, effectively collapsing the representation to a one-dimensional subspace. This allows rank-one subcomponents to capture full features without needing to split them across multiple subcomponents, thus maintaining interpretability and faithfulness to the original SPD formulation.
the model musn't use all $d_{fi}=2$ dimensions o
...
\subsection{TMS with paired features}
\label{sec:tms-tied-experiment}

The baseline established that softmax-and-attention recovers the correct number of independent features but does not perfectly concentrate their magnitudes. This experiment extends to the simplest dependent case: features that always co-occur in pairs, creating statistical dependence between their activations.

The architecture is unchanged from Section~\ref{sec:spd-independent-features}; only the input distribution changes. Features are partitioned into pairs $\{f_{2i-1}, f_{2i}\}_{i=1}^{20}$, with each pair zero with probability $1-p$, and otherwise each pair member drawn independently from $\mathcal{U}(\epsilon, 1)$. The setup uses $d_{\text{features}} = 40$ and $d_{\text{hidden}} = 10$.

Each paired feature constitutes a 2-dimensional irreducible feature in the sense of Section~\ref{sec:irreducibility}: its joint distribution is neither separable, since the two components co-occur, nor a disjoint mixture. The multi-dimensional superposition hypothesis allows the model to represent such features in a subspace of dimension up to $d_{f_i} = 2$, but does not enforce the use of all dimensions when the geometry permits a lower-rank embedding.

The trained model exploits this freedom. Cosine similarity within and across pairs (Table~\ref{tab:tms-paired-cosine-similarity}) shows near-perfect collinearity within pairs and approximate orthogonality across them. The model has collapsed each 2-dimensional feature into a 1-dimensional subspace, encoding each pair along a single shared direction.
...
This collapse places the ground-truth decomposition within the reach of rank-1 subcomponents: each pair should be captured by a single subcomponent whose direction matches the shared encoder column. The experiment isolates the question of whether each method's importance function correctly groups co-occurring features into the same subcomponent, rather than splitting them across two. Both decompositions are trained at rank $1$ with $C = 150$ subcomponents, matching the model's effective representation.

\subsubsection{Grouping by causal importance}

The expected pattern of causal importance values is straightforward: for each pair $\{f_{2i-1}, f_{2i}\}$, both feature instances should activate the same subcomponent. A method that splits paired features across two subcomponents would show a two-hot pattern within each pair; one that correctly groups them would show a single-hot pattern shared across the pair members.

[insert importance comparison figure]

[discussion: SPD groups / fails to group, softmax-and-attention groups / fails to group, with quantitative summary if useful]

\subsubsection{Cross-pair separation}

As a secondary check, the recovered subcomponents are tested for $\delta$-orthogonality across pairs. The encoder's across-group cosine similarity ($0.053$) provides the reference: a faithful decomposition should reproduce this separation in its $V_c$ matrices.

\begin{equation}
    M_{cc'} = \frac{V_c^\top V_{c'}}{\|V_c\| \|V_{c'}\|}
\end{equation}

[brief comparison table or single figure, kept short — full $V$ alignment analysis is deferred to Section~\ref{sec:geometric-experiment}]
...
important to mention that the structure that spd decomposes into is not clearsometimes one hots
ome
