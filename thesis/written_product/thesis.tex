...
However, both identifying an appropriate decomposition
 and the decomposition into these atomic units remain open problems.
...
subcomponents in the same layer, to encourage that dependent features are grouped within one "mechanism".
...
The superposition hypothesis states that the hidden states can be represented as a linear combination of sparse one-dimensional features. \cite{elhage2022toymodelssuperposition,engels2025languagemodelfeaturesonedimensionally} Concretely, the hidden state 
...
\begin{itemize}
    \setlength\itemsep{0em}
    \item \textbf{Statistical independence}: $p(a, b)= p(a) \, p(b)$
    \item \textbf{Sum of disjoint distributions}: $p(a, b) = w \, p(a) \, \delta(b) + (1 - w) \, p'(a, b)$
\end{itemize}
where $\delta$ is the Dirac delta distribution and either a or b is lower dimensional in the disjoint distribution. 
...
require columns to be linearly independent: $d_{f_i}$ specifies
an upper bound on the representational capacity of $V_{i}$, while
$\text{rank} (V_i) \leq d_{fi}$, determines the effective intrinsic dimensionality
required to represent the feature.. For example, a model may align dependent
components within a shared lower-dimensional subspace
(Section \ref{fig:tms-paired-comparison}).
...
A decomposition into lower-dimensional subcomponents, necessarily forces features to be split across multiple subcomponents, or to be contained in a single subcomponent at the cost of faithfulness.
...
then has rank at most $k$, recovering the original SPD definition when $k=1$.
...
as it will be discussed in Section~\ref{sec:svd_reformulation}.


In SPD, each subcomponent’s importance is computed independently through a learned function followed by a sigmoid nonlinearity,
allowing multiple subcomponents to be active for a given input and thus distribute a
feature across them. The minimality loss does not translate to the rank-k setting, where each subcomponent can represent multiple feature directions:
it imposes no penalty for distributing a feature across multiple subcomponents,
nor for a single subcomponent to encode parts of multiple features. (Section \ref{sec:spd_losses})

To address this, importance is redefined as a relational quantity over all subcomponents in the same layer.
\[
    \alpha^l_c(x) = p\!\left(c \mid \{h^l_{c'}(x)\}^C_{c'=1}\right), \quad h^l_c(x) = {V^l_c}^{\top} \vec{a}^l(x) \in \mathbb{R}^k
\]
where $\vec{a}^l(x)$ is the activation of layer $l$ for input $x$.
The distribution is computed using a transformer block operating over subcomponents, followed by a softmax over the C components.


This produces a normalized assingmnent over subcomponents, $\sum_{c=1}^{C}\alpha^l_c(x) = 1$,
...
rank-$k$ extension. If $k$ is too small or the minimality weight is too
low, the faithfulness loss dominates and forces features to split across
...
A limitation of this formulation is its computational cost. Computing
