...
Additionally the paper provides a formal definition of irreducibility and a corresponding refinement of the superposition hypothesis.
...
Under this multi-dimensional view, the superposition hypothesis,
the hidden states are written as a sum of feature contributions:
\begin{equation}
...
The multi-dimensional hypothesis makes two structural commitments
about the model's representation. First, the projection matrices
$V_i$ are pairwise $\delta$-orthogonal, with the column spaces of
different features approximately disjoint in hidden space. Second,
...
$V_i$ without loss of reconstructive information.
...
determined by how the model has chosen to encode the feature. The
rank required by a parameter decomposition method to host an
irreducible feature as a single unit is the effective
representational rank, not the formal dimensionality.

\paragraph{Implications for parameter decomposition.}
This framework offers a natural framing for the question raised in
the introduction of which level a parameter decomposition should
target. Under the multi-dimensional superposition hypothesis, the
irreducible feature is the unit at which the model manipulates
structure, and a decomposition that matches this granularity stays
faithful to the model's own organisation. Finer decompositions
split irreducible features into coordinates the model treats as a
single object; coarser ones collapse distinct features into one.
The present work takes the irreducible feature as the unit its
extension aims to recover.

SPD's rank-one parameterisation can host an irreducible feature as
a single unit only when its effective representational rank is one;
the extension developed in the next section accommodates features
whose effective representational rank is greater.
