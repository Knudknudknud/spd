...
Recent work shifts focus from matching activations \autocite{sparseautoencoderscanonicalunits} to directly decomposing parameters. Guided by the minimum description length, these methods aim to recover components that are faithful to the original weights, sparse per input, and individually simple. 
...
However, both the problem of identifying an appropriate decomposition
and characterizing its atomic units remain open. \autocite{sharkey2025openproblemsmechanisticinterpretability}
Whether a parameter decomposition recovers the computational primitives
of an neural network depends on what the ground-truth decomposition
...
decomposed either into components corresponding to individual side
...
\autocite{engels2025languagemodelfeaturesonedimensionally} introduces a
a formal definition of irreducibility to disambiguate cases such
as the cube example.
Features are irreducible, and thus constitute the unit of decomposition,
if their joint distribution cannot be decomposed into independent
factors or into a mixture containing a lower-dimensional component. Empirically,
it also demonstrates that cyclical concepts, such as days of the week are represented as irreducible two-dimensional features in transformer models.
...
either a feature-level or a structural-level unit \autocite{engels2025languagemodelfeaturesonedimensionally}. 
A reformulation of the importance function is proposed, in which importance
corresponds to the conditional probability that a given subcomponent
is responsible for a computation, conditioned on the activations of all
subcomponents in the same layer. Finally, it is shown that the
parameterisation must be extended to rank-k subcomponents to accommodate
...
\begin{equation}
    \mathcal{L}_{\text{minimality}} = - \frac{1}{|C|} \sum_{l=1}^{L} \sum_{c=1}^{C} \alpha^l_c(x) \log \alpha^l_c(x)
\end{equation}
