...
Add the part about how We get one-hot importance values,
but they don't seem to contribute to the 

input * output reading plot.

Could this be because of U? that is technically not penalized
also comment on the norm.


I think this is the solution:
After the first hidden layer,
the projection lives in a 50 dimensional subspace, but we 
have an overcomplete basis with the 100 subcomponents,
necessarily these cannot project into 100 orthogonal directions,
so they must contribute to multiple features.


\subsubsection{Toy model of compressed computation}
The Toy Model of Compressed Computation \autocite{bhagat2025compressed,stochasticparameterdecomposition,AttributionBasedParameterDecomposition},
is a toy model trained to learn the function $y_{i}= x_{i}+relu(x_{i})$,
consisting of an embedding matrix, $W_{E}$, uenembedding matrix, $W_{U}=W_{E}^T$,
100 input features and a residual MLP, $MLP_{in} \in \mathbb{R}^{d_{50},d_{embedding}}$, and similarly $MLP_{out} \in \mathbb{R}^{d_{embedding},d_{50}}$.

The full computation becomes:
\[
    y =W_{U} (W_{E}(x) + W_{in}(\mathrm{ReLU}(W_{E}(x))))
\]

A naive solution to this problem is to learn a monosemantic decomposition, where
each neuron in the MLP is responsible for computing one relu activation.
However this will perform poorly on the task, as 50 neurons are forced to guess.

To compare results with the original SPD decomposition,
$d_{features}= 100,$
$d_{mlp}= 50,$
$C = 200$.
Predictably, for the $MLP_{in}$ the ground truth mechanism should be to have each subcomponent learn one feature each,
however for the $MLP_{out}$ the ground truth mechanism is substantially less clear.
Originally presented in APD \autocite{AttributionBasedParameterDecomposition},
they found that the output layer, also learnt a monosemantic representation,
where each subcomponent was responsible for one feature. Whereas SPD \autocite{stochasticparameterdecomposition},
learns to capture the output layer within 50 subcomponents, where each
contributes to the computation of multiple features, and the remaining 150 subcomponents are pruned to near-zero norm.


\subsection{Sampling effect on loss}


We first take the chance to look at how the sampling distribution affects the decomposition for SPD and SOftmax.
We compare the one-hot sampling to the randomly initialized one.

Notably we find that Softmax, produces greatly lower losses under one-hot sampling,
most certainly because we aim to maximize the probability of ownership,
and one-hot sampling removes any noisy or even meaningless features that may come from random activaiton.
sigmoids on the other hand do not have this problem, as they do not enforce competition,but rather
if the parts together sum up to be important. We will later see a consequence of this, when exploring the toy model of geometry,
where the features become multi-dimensional, and thus require to be grouped together.
...
Layerwise stochastic reconstruction & & & & \\
\bottomrule
\end{tabular}
\caption{Reconstruction quality across sampling regimes for the Residual MLP. \textit{To be filled with final numbers.}}
...
In the case of our softmax variant, we also manage to replicate the results of APD,
where we manage to find almost one hot decomposition for both the input and output layers,
however this interpretation is likely to be misleading.

Plot of causal importances.

Notably, we train the causal importance function to predict the importance of a subcomponent based on
$V^T(x)$, the layer input projected into the subcomponents "input" reading.

However, by computing the neuron contribution of each subcomponent, for each feature,
we find that the subcomponents manages to produce meaningful contirbution to more than one feature each,
despite achieving one-hot importance values. Initially this seems paradoxal, as 
how can a mechanism be completely ablated for one feature, while still contributing meaningful weight to it's output space.

Plot of activation contribution.

This is likely a consequence of the overcomplete basis that the 100 subcomponents learn,
namely that if 100 subcomponents learn to project into the same 50 dimensional subspace through a linear mapping,
they must necessarily share output direction, and thus contribute to multiple features, even if the importance function assigns them to a single feature.

Plot of cosine similarity of U vectors.


\subsubsection{Differences in norm}


A second experiment we would like to run is to see, how softmax affects the norms of the subcomponents,
likely we hypothesize that a more expressive MLP may learn to disentangle neurons that contribute similarly,
and may not even need to shrink the norm to maximize the probability of ownership, as we do not directly enforce the norm to shrink.
We show the difference in norms between SPD and Softmax in the figure below.
Another hypothesis for this could be that the softmax does not manage to produce "hard zeroes", which was the reason the original
SPD paper chose to use hard sigmoid gating instead. However Plotting the causal importances of the subcomponents,
clearly shows that this is not the case, likewise emperical results, with sparsemax produce the same results. \autocite{peters2019sparsesequencetosequencemodels}


Placeholder plot for causal importance frequency.

Interestingly as expected, all subcomponents produce substantially lower norms, after the first 50 subcomponents.
However the norm distribution after this drop off differs substantially between the two methods.
In particular, SPD manages to prune the norm to zero for all remaining subcomponents,
where as softmax has many subcomponents with low norm, without being causally important for any features.

Referencing back to the TMS experiment, this is likely part of the explanation for why we observed significant feature shrinkage.
The faithfulness loss, is trained to minimze the MSE between the layers weight matrix,
while it does not care where it is distributed, as long as they sum up to be equal.
That means if the minimality loss only partially ablates the weights to be "insignificant",
albeit non-zero, the faithfulness loss will not reassign the missing norm to
the dominant subcomponents.


\subsubsection{Cause of the lying?}

Returning to the question of one-hot importance, we fear that this may be an illusion of interpretability,
or while not an illusion it is misguiding. The model has very likely learnt
to disambiguate the subcomponents, but the subcomponents or perhaps particularly their
U matrices, are not constrained to project orthogonally to eachother and
are not represented in this importance calculation.



Also need to relate back to the multidimensional feature hypothesis.



\subsubsection{Sampling regimes}

We compare two sampling regimes for the masks during stochastic reconstruction: \textbf{one-hot sampling}, where exactly one subcomponent is forced active per feature, and \textbf{at-least-zero sampling}, the relaxed regime used during normal training. Both regimes converge to similar reconstruction quality, but they place different pressures on the structure of the recovered decomposition.
...
The softmax variant achieves near-perfect one-hot causal importance, with each feature routing essentially all its mass to a single subcomponent (Figure~\ref{fig:resid_mlp_ci}). The histogram of importance values is bimodal at 0 and 1, with negligible off-peak mass --- the same property the original SPD method achieves through its sigmoid-and-minimality combination, here recovered through softmax-with-entropy.

\begin{figure}[h]
\centering
% \includegraphics[width=\textwidth]{figures/resid_mlp_causal_importance.png}
\caption{Causal importance values for the softmax variant on the Residual MLP. \textit{Left:} heatmaps for $W_{\text{in}}$ and $W_{\text{out}}$, showing near-diagonal one-hot routing of features to subcomponents. \textit{Right:} histogram of all importance values, bimodal at 0 and 1.}
\label{fig:resid_mlp_ci}
\end{figure}

This is a non-trivial result on its own. Replacing the importance minimality loss with entropy preserves the routing property that SPD's heatmaps are known for, suggesting that the routing structure can be recovered without the Schatten-style parameter penalty that the original method relies on.

\subsubsection{Neuron contributions}

Following \cite{Bra+25}, we visualize the per-neuron contribution of each subcomponent compared to the target model's per-feature neuron contributions. For each feature, we identify the subcomponent with the largest neuron contribution (the ``dominant'' subcomponent) and plot its full neuron pattern alongside the target's.

\begin{figure}[h]
\centering
% \includegraphics[width=\textwidth]{figures/resid_mlp_neuron_contributions.png}
\caption{Per-neuron contributions for the Residual MLP. \textit{Top:} target model contributions for the first 30 features, with the top-contributing neurons labelled. \textit{Bottom:} the dominant SPD subcomponent for each feature, with neuron labels propagated from the target panel.}
\label{fig:resid_mlp_neurons}
\end{figure}

The dominant subcomponents recover the correct neurons for each feature, matching the target's labels. However, the magnitudes are visibly shifted: the subcomponent contributions do not consistently reproduce the target's magnitudes, with the same neuron often appearing across multiple subcomponents at non-trivial weight. Despite clean routing, the parameter directions are not isolated to single features.

This is a different failure mode than TMS exhibits. In TMS the softmax variant produced shrinkage on the dominant subcomponents themselves; here, the dominant subcomponents are approximately correct, but additional subcomponents carry residual weight on the same neurons.

\subsubsection{Subcomponent norms}

To make this concrete, we plot the Frobenius norm $\|A_c B_c\|_F$ of each rank-1 subcomponent, sorted by magnitude (Figure~\ref{fig:resid_mlp_norms}).

\begin{figure}[h]
\centering
% \includegraphics[width=\textwidth]{figures/resid_mlp_subcomponent_norms.png}
\caption{Sorted Frobenius norms $\|A_c B_c\|_F$ of all $C = 200$ subcomponents for $W_{\text{in}}$ (left) and $W_{\text{out}}$ (right). A clear cliff separates roughly 80 high-norm subcomponents from a long tail of $\sim 120$ low-but-nonzero subcomponents.}
\label{fig:resid_mlp_norms}
\end{figure}

The distribution is bimodal but not clean. The first $\sim 80$ subcomponents sit at full norm ($\sim 1.0\text{--}1.4$ for $W_{\text{in}}$, $\sim 2.0$ for $W_{\text{out}}$), followed by a sharp drop, and then a long tail of $\sim 120$ subcomponents at norm $0.15\text{--}0.35$. The tail is small relative to the head but unambiguously \textbf{non-zero} --- these subcomponents have not been pruned.

This contrasts with the TMS$_{5\text{-}2}$ result, where the softmax variant correctly pruned the unused subcomponents to near-zero norm despite the ML2R shrinkage on the active ones. In the Residual MLP setting, neither property is fully recovered: active subcomponents experience some magnitude drift, and inactive subcomponents are not driven to zero.
